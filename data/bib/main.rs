#![allow(uncommon_codepoints)]use std::collections::{BTreeMap,btree_map::Entry};
use{String as Str,Some as S,None as N,println as pr,unimplemented as die,matches as m};
use{Iterator as I,Option as O,std::{fs,fs::File as F,io::Write,iter::Peekable as Pk}};
...
type R<O=()>=anyhow::Result<O>;type M=BTreeMap<Str,Str>;
const L:&str="links.bin";const B:&str="bibliography";
fn load()->R<M>{Ok(fs::read_to_string(L)?.lines().flat_map(|l|{
	let(k,v)=l.split_once('\0')?;S((k.into(),v.into()))}).collect())}
...
fn main()->R{let mut m=load().unwrap_or_default();let ps=std::env::args().skip(1).map(|a|
	R::Ok(post(&std::fs::read_to_string(&a)?,&mut m)?)).collect::<R<Vec<_>>>()?;
	save(&m)?;for p in ps{
		pr!("---\n## {B}\n");for l in p{
			pr!("- [{}]({l})",m.get(&l).map(|s|&**s).unwrap_or(""))}}Ok(())}
fn bibʔ(e:O<&E>)->bool{m!(e,S(E::Text(t))if t.eq_ignore_ascii_case(B))}
...
fn post(src:&str,m:&mut M)->R<Vec<Str>>{let mut p=P::new_ext(src,PO::all()).peekable();
	let mut b=false;let mut v=vec![];while let S(e)=p.next(){
		if m!(e,E::Start(T::Heading{..})){b=bibʔ(p.peek())}
		else if m!(e,E::Start(T::FootnoteDefinition(..))){b=false}
		let E::Start(T::Link{link_type:ty,dest_url:d,title:t,id})=e else{continue};
		if!d.starts_with("http"){continue;}
		let mut t=t.into();let k=d.split_once('#').map_or(&*d,|t|t.0).into();name(&mut t,&id,ty,&mut p);
		if b{m.insert(k,t);}else{
			...
			v.push(k);}}Ok(v)}
fn infer(u:&str)->O<Str>{let h=H::parse_document(&get(u).inspect_err(|e|{dbg!(e);}).ok()?);
	match(sel("meta[name=author]",&h).and_then(|n|n.attr("content")),sel("title",&h).map(|n|n.text().collect::<String>())){
		(S(a),S(t))=>S(format!("{a}, “{t}”")),
		(S(a),N   )=>S(format!("{a}")),
		(N   ,S(t))=>S(format!("“{t}”")),
		(N   ,N   )=>N}}
fn get(u:&str)->R<Str>{Ok(ureq::get(dbg!(u)).call()?.body_mut().read_to_string()?)}
